  ...
  const handleCreateAssignment = async (e) => {
    ...
    try {
      ...
      for (const operadorId of selectedOperators) {
        ...
        // 🔔 NUEVO: crear notificación interna para el operador
try {
  const op = operators.find((o) => o.id === operadorId || o.code === operadorId);
  ...
  if (operatorEmail) {
    await fetch('/api/notifications', {
      ...
      body: JSON.stringify({
        ...
        message: `Hola ${operatorName}, se te asignó la actividad "${data.activity}" en ${data.location}.`,
        type: 'assignment_created',
        related_id: createdRef.id,
        related_module: 'assignments',
      }),
    });
  } else {
    console.warn('No se encontró correo del operador para notificación:', operadorId, op);
  }
} catch (notificationErr) {
  console.error('Error creando notificación interna para operador:', notificationErr);
}
...
{/* 🔔 BLOQUE DE NOTIFICACIONES INTERNAS PARA ADMIN */}
<section
  style={{
    ...styles.notificationsContainer,
    ...(isMobile ? styles.notificationsContainerMobile : {}),
  }}
>
  <div style={styles.notificationsInfo}>
    <h3 style={styles.notificationsTitle}>🔔 Notificaciones internas</h3>
    <p style={styles.notificationsText}>
      Revisa solicitudes nuevas, préstamos pendientes y novedades recientes del sistema.
    </p>
  </div>
